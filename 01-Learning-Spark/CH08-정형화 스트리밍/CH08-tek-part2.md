...
- **장점** — 구현이 단순하고 Spark가 상태를 자동 관리. 전체 기간 누적 집계를 추가 로직 없이 표현 가능
- **단점** — 키 카디널리티가 높을 경우 StateStore가 무한 증가해 OOM 위험. 상태 삭제 시점을 사용자가 제어할 수 없음
...
- **장점** — 처리 지연과 무관하게 이벤트 발생 시각 기준 정확한 집계. 워터마크로 상태 자동 정리. 텀블링/슬라이딩 두 가지 윈도우 패턴 지원
- **단점** — append 모드 사용 시 워터마크 경과 후에야 결과가 출력되어 지연 발생. 슬라이딩 윈도우는 이벤트가 여러 윈도우에 중복 집계되어 연산 비용 증가. 워터마크 임계값이 짧으면 데이터 유실, 길면 상태 증가의 트레이드오프 존재
...
- **장점** — StateStore 자동 정리로 메모리 누수 방지. append 모드와 결합해 "확정된 결과만 출력" 패턴 구현 가능
- **단점** — 워터마크 기준선보다 늦게 도착한 이벤트는 무조건 드롭되어 데이터 손실 가능. 분산 환경에서 느린 파티션의 이벤트 타임이 전체 워터마크 기준선을 지연시킬 수 있음
...
출력 모드별 동작:
  update  모드  ->  윈도우 결과가 갱신될 때마다 출력, 기준선 보다 과거일 경우 state 삭제
  append  모드  ->  워터마크가 윈도우 종료 시각을 지난 후에만 결과 출력 (늦은 데이터 보장, 이벤트타임 윈도우 집계에만 사용 가능)
...
- **장점** — StateStore가 전혀 필요 없어 메모리 효율이 우수. 일반 배치 조인과 동일한 문법. `broadcast()`로 셔플 완전 제거 가능
- **단점** — 정적 DF는 쿼리 시작 시 1회만 로드되므로 참조 데이터 변경이 즉시 반영되지 않음 (쿼리 재시작 필요). 좌/우 outer join 방향 제약 존재
...
- **장점** — 두 스트림 간 지연 도착 이벤트까지 버퍼에서 처리하여 정확한 이벤트 매칭 가능. 비즈니스상 자연스러운 "노출 후 클릭", "주문 후 결제" 패턴을 직접 표현 가능
- **단점** — 양쪽 버퍼를 StateStore에 유지해야 하므로 메모리 사용량이 큼 (버퍼 크기 = 워터마크 + 시간 범위). full outer join 미지원. outer join 사용 시 워터마크와 시간 범위 조건이 필수이므로 설계 복잡도 증가
...
- **장점** — 어떤 비즈니스 로직도 상태로 표현 가능. 그룹별 독립적인 상태 생명주기 관리. ProcessingTime/EventTime 두 가지 타임아웃 전략 지원
- **단점** — 상태 정리(타임아웃 처리, `state.remove()`)를 사용자가 직접 구현해야 함. 로직 누락 시 StateStore 메모리 누수 발생. managed 연산보다 코드 복잡도가 현저히 높음
...
- **장점** — 매 배치마다 각 그룹의 현재 상태를 출력하여 실시간 현황 모니터링에 적합. 1건 반환 보장으로 다운스트림에서 결과 수신 여부 예측 용이
- **단점** — 반드시 1건을 반환해야 하므로 "이번 배치에 결과 없음" 표현 불가. update 모드만 지원하여 append 모드(DW 최종 적재) 불가. 타임아웃 발생 시 별도 결과를 만들려면 flatMapGroupsWithState로 전환 필요
...
- **장점** — ProcessingTimeTimeout은 워터마크 없이 사용 가능해 단순 비활성 감지에 적합. EventTimeTimeout은 이벤트 타임 기준 정확한 세션 만료 구현 가능
- **단점** — 타임아웃은 배치 실행 시점에 체크되므로, 배치 간격이 길면 만료 감지가 늦어짐. EventTimeTimeout은 워터마크 필수이며 느린 파티션에 의해 워터마크가 지연되면 타임아웃도 함께 지연됨. 타임아웃 체크 및 `state.remove()` 로직을 사용자가 직접 구현해야 함
...
- **장점** — append 모드 지원으로 DW 최종 적재에 적합. 0건 반환으로 "이번 배치 출력 없음" 표현 가능. 조건 충족 시에만 이벤트 방출하는 실시간 알림 패턴 구현 가능
- **단점** — `mapGroupsWithState`보다 반환 건수 관리 로직이 추가되어 코드 복잡도 증가. yield 누락 또는 과다 yield 시 결과 누락/중복이 발생하며 런타임 오류 없이 조용히 실패할 수 있음
